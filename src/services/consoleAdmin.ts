...
export async function reserveConsoleWorkspacePlacement(
  ...
  return apiFetch<ConsolePlacementRequest>(buildApiUrl(PLACEMENT_REQUESTS_BASE), {
    method: 'POST',
    headers: { 'X-Platform-Change-Reason': reason, 'X-Idempotency-Key': createIdempotencyKey() },
    body: JSON.stringify({ confirm: true, workspace_id: workspaceId, target_cluster_id: targetClusterId, expected_placement_fence: expectedPlacementFence })
  })
}

/** Release an unconsumed capacity reservation and fence out a delayed placement worker. */
export async function rollbackConsolePlacementReservation(placementRequestId: string, reason: string): Promise<ConsolePlacementRequest> {
  return apiFetch<ConsolePlacementRequest>(buildApiUrl(`${PLACEMENT_REQUESTS_BASE}${encodeURIComponent(placementRequestId)}/rollback/`), {
    method: 'POST',
    headers: { 'X-Platform-Change-Reason': reason, 'X-Idempotency-Key': createIdempotencyKey() },
    body: JSON.stringify({ confirm: true })
  })
