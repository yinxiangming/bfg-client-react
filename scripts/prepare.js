...
function syncPluginRoutes() {
  ...
  if (fs.existsSync(PLUGINS_DIR)) {
    ...
    for (const plugin of plugins) {
      ...
      writeRouteGuards(plugin, syncedFiles)
    }
    if (plugins.length > 0) {
...
// A plugin's storefront and account pages answer 404 while its extension is switched off.
// Only the folders that belong to the plugin alone are wrapped. A route merged into a shared
// folder ((storefront)/<name>/ or account/<name>/, where name is not the plugin id) shares its
// layout with core pages and is left unguarded.
// Admin pages are covered in the admin shell, which knows the workspace only in the browser.
function writeRouteGuards(plugin, syncedFiles) {
  const guardedDirs = [
    path.join(APP_DIR, '(storefront)', 'plugins', plugin),
    path.join(APP_DIR, 'account', 'plugins', plugin),
  ]
  for (const dir of guardedDirs) {
    if (!fs.existsSync(dir)) continue
    const ownLayout = ['layout.tsx', 'layout.ts', 'layout.jsx', 'layout.js']
      .map((name) => path.join(dir, name))
      .find((file) => fs.existsSync(file))
    if (ownLayout) {
      console.warn(
        `Plugin ${plugin} ships ${path.relative(SRC_DIR, ownLayout)}: render its children inside ` +
          'ExtensionRouteGuard so these pages stay hidden while the extension is switched off.'
      )
      continue
    }
    const guard = path.join(dir, 'layout.tsx')
    fs.writeFileSync(guard, routeGuardSource(plugin))
    syncedFiles.push(path.relative(SRC_DIR, guard))
  }
}
